...
async function newGame(level) {
	...
	let temp = await generateSudoku(level);
	 currentHints = 0;
 maxHints = hintClicks[level];
	board = temp.grid;
	ans = temp.solution;
	let error = document.getElementById("error-count");
	
	error.innerText = 0;
	let hintscount = document.getElementById("hints-count");
	
	hintscount.innerText = `${currentHints}/${maxHints}`;
	let minute = document.getElementById("minute");
	let sec = document.getElementById("second");
minute.innerText = 0;
sec.innerText = 0.0;
...
const level = "easy"; // You can dynamically set this
...
function Move(sourceID, destID, text) {
    ...
    hintBtn.innerText = "🔍";
    hintBtn.classList.add("hint-icon");
    hintBtn.style.cursor = "pointer";
    hintBtn.style.fontSize = "14px";
  ...
  if (currentHints >= maxHints) {
    Swal.fire({
      ...
      text: `You can only use ${maxHints} hints.`,
    });
    return;
  }
  ...
  const result = await Swal.fire({
    title: "Use a Hint?",
    text: "Do you want to reveal the correct value for this cell?",
    icon: "question",
    showCancelButton: true,
    confirmButtonText: "Yes, Show Hint",
    cancelButtonText: "No",
    reverseButtons: true
  });
  ...
  const [x, y] = dest.id.split("-").map(Number);

  try {
    const res = await fetch(`http://127.0.0.1:5000/hint?x=${x}&y=${y}`);
    const data = await res.json();

    const correctValue = data.hint;
	dest.classList.remove("wrong");
    dest.innerText = correctValue;
    //dest.style.color = "white";
    dest.classList.add("hint");
    hintBtn.remove();
    ...
    Swal.fire({
      title: "Hint Used!",
      text: `The correct value is ${correctValue}`,
      icon: "success",
      timer: 1500,
    ...
    Swal.fire({
      icon: 'error',
      title: 'Oops!',
      text: 'Failed to fetch hint from server.'
    });
  }
});
